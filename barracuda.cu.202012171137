...
//	if ( bwt != 0 )
	{
		...
		*forward_seq_len = bwt_src->seq_len;
		if(mem_available > 0)
		{
			...
			hipMalloc((void**)bwt, bwt_src->bwt_size*sizeof(uint32_t));
			...
			//hipBindTexture(0, bwt_occ_array, *bwt, bwt_src->bwt_size*sizeof(uint32_t));
			hipMalloc((uint4**)&bwt_occ_array, bwt_src->bwt_size*sizeof(uint32_t));
			hipMemcpy (bwt_occ_array, *bwt , bwt_src->bwt_size*sizeof(uint32_t), hipMemcpyDeviceToDevice);
...
//	if ( rbwt != 0 )
	{
		...
		if (mem_available > 0)
		{
			*backward_seq_len = bwt_src->seq_len;
			...
			//Allocate memory for bwt
			hipMalloc((void**)rbwt, bwt_src->bwt_size*sizeof(uint32_t));
			//copy reverse bwt occurrence array from host to device and dump the bwt to save CPU memory
			hipMemcpy (*rbwt, bwt_src->bwt, bwt_src->bwt_size*sizeof(uint32_t), hipMemcpyHostToDevice);
			//bind global variable bwt to texture memory rbwt_occ_array
			//hipBindTexture(0, rbwt_occ_array, *rbwt, bwt_src->bwt_size*sizeof(uint32_t));
			hipMalloc((uint4**)&rbwt_occ_array, bwt_src->bwt_size*sizeof(uint32_t));
		        hipMemcpy (rbwt_occ_array, *rbwt , bwt_src->bwt_size*sizeof(uint32_t), hipMemcpyDeviceToDevice);
			//copy bwt structure data to constant memory bwt_cuda structure
			hipMemcpyToSymbol(HIP_SYMBOL(rbwt_cuda), bwt_src, sizeof(bwt_t), 0, hipMemcpyHostToDevice);
			bwt_destroy(bwt_src);
		}
		else
		{
...
void free_bwts_from_cuda_memory( unsigned int * bwt , unsigned int * rbwt )
{
	if ( bwt != 0 )
	{
		//hipUnbindTexture(bwt_occ_array);
	        hipFree(bwt_occ_array);
		hipFree(bwt);
	}
	if ( rbwt != 0 )
	{
		//hipUnbindTexture(rbwt_occ_array);
                hipFree(rbwt_occ_array);
		hipFree(rbwt);
	}
}
...
int copy_sequences_to_cuda_memory ( bwa_seqio_t *bs, uint2 * global_sequences_index, uint2 * main_sequences_index, unsigned char * global_sequences, unsigned char * main_sequences, unsigned int * read_size, unsigned short & max_length, int mid, int buffer)
{
	...
	hipFree(sequences_index_array);
        ...
        hipMalloc((uint2**)&sequences_index_array, (number_of_sequences)*sizeof(uint2));
        hipMemcpy (sequences_index_array, global_sequences_index , (number_of_sequences)*sizeof(uint2), hipMemcpyDeviceToDevice);
    ...
    hipMalloc((unsigned int**)&sequences_array, (1ul<<(buffer))*sizeof(unsigned char));
    hipMemcpy (sequences_array, global_sequences , (1ul<<(buffer))*sizeof(unsigned char), hipMemcpyDeviceToDevice);
...
__device__ unsigned char read_char(unsigned int pos, unsigned int * lastpos, unsigned int * data , unsigned int *sequences_array)
...
// which is packed as half bytes and stored as in a unsigned int array
{
	...
	{
		*data = tmp = sequences_array[pos_shifted];
		*lastpos=pos_shifted;
	}
...
#define BWTOCC(a) (bwt_occ_array[a])
#define RBWTOCC(a) (rbwt_occ_array[a])

__device__ uint4 bwt_cuda_occ4(bwtint_t k,uint4 * bwt_occ_array)
...
__device__ bwtint_t bwt_cuda_occ(bwtint_t k, ubyte_t c,uint4 * bwt_occ_array)
// return occurrence of c in bwt with k smallest suffix by reading it from texture memory
{
	#if BWT_TABLE_LOOKUP_ENABLE == 1
	uint4 ok = bwt_cuda_occ4(k,bwt_occ_array);
	switch ( c )
	{
	case 0:
		return ok.x;
...
__device__ uint4 rbwt_cuda_occ4(bwtint_t k,uint4 * rbwt_occ_array)
// return occurrence of c in bwt with k smallest suffix by reading it from texture memory
{
	// total number of character c in the up to the interval of k
	uint4 tmp;
...
__device__ inline bwtint_t rbwt_cuda_occ(bwtint_t k, ubyte_t c,uint4 * rbwt_occ_array )
// return occurrence of c in bwt with k smallest suffix by reading it from texture memory
{
	#if BWT_TABLE_LOOKUP_ENABLE == 1
	uint4 ok = rbwt_cuda_occ4(k,rbwt_occ_array);
	switch ( c )
	{
	case 0:
		return ok.x;
...
__device__ void bwts_cuda_2occ(unsigned int k, unsigned int l, unsigned char c, unsigned int *ok, unsigned int *ol, unsigned short bwt_type,uint4 * bwt_occ_array,uint4 * rbwt_occ_array)
{
	unsigned int _k, _l;
	unsigned int i;
	// tmp variable
	...
	{
		if (k == l) {
			*ok = *ol = bwt_cuda_occ(k, c,bwt_occ_array);
			return;
		}
		...
		{
			*ok = bwt_cuda_occ(k, c,bwt_occ_array);
			*ol = bwt_cuda_occ(l, c,bwt_occ_array);
			return;
		}
		k = _k;
		l = _l;
	...
	{
		if (k == l) {
			*ok = *ol = rbwt_cuda_occ(k, c,rbwt_occ_array);
			return;
		}
		...
		{
			*ok = rbwt_cuda_occ(k, c,rbwt_occ_array);
			*ol = rbwt_cuda_occ(l, c,rbwt_occ_array);
			return;
		}
		k = _k;
		l = _l;
...
__device__ void bwt_cuda_device_calculate_width (unsigned char* sequence, unsigned short sequence_type, unsigned int * widths, unsigned char * bids, unsigned short length,uint4 * bwt_occ_array,uint4 * rbwt_occ_array)
//Calculate bids and widths for worst case bound, returns widths[senquence length] and bids[sequence length]
{
	...
	for (i = 0; i < length; ++i) {
		...
		if (c < 4) {
			#if BWT_2_OCC_ENABLE == 1
			//unsigned int ok, ol;
			//bwts_cuda_2occ(k - 1, l, c, &ok, &ol, bwt_type,bwt_occ_array);
			uint4 ok, ol;
			...
			if ( bwt_type == 0 )
			{
				k = bwt_cuda.L2[c] + bwt_cuda_occ(k - 1, c,bwt_occ_array) + 1;
				l = bwt_cuda.L2[c] + bwt_cuda_occ(l, c,bwt_occ_array);
			}
			else
			{
				k = rbwt_cuda.L2[c] + rbwt_cuda_occ(k - 1, c,rbwt_occ_array) + 1;
				l = rbwt_cuda.L2[c] + rbwt_cuda_occ(l, c,rbwt_occ_array);
			}
			#endif // BWT_2_OCC_ENABLE == 1
		}
		if (k > l || c > 3) {
...
__device__ void bwt_cuda_device_calculate_width_limit (init_info_t* init_info,unsigned char* sequence, unsigned short sequence_type, unsigned int * widths, unsigned char * bids, unsigned short length,uint4 * bwt_occ_array)
//Calculate bids and widths for worst case bound, returns widths[senquence length] and bids[sequence length]
{
	unsigned short bid;
	//suffix array interval k(lower bound) and l(upper bound)
...
__device__ int bwt_cuda_match_exact(unsigned short bwt_type, unsigned int length, const unsigned char * str, bwtint_t *k0, bwtint_t *l0,uint4 * bwt_occ_array,uint4 * rbwt_occ_array)
//exact match algorithm
{
	...
	for (i = length - 1; i >= 0; --i)
	{
		...
		#if BWT_2_OCC_ENABLE == 1
		unsigned int ok, ol;
		bwts_cuda_2occ(k - 1, l, c, &ok, &ol, bwt_type,bwt_occ_array,rbwt_occ_array);
		if ( bwt_type == 0 )
		{
...
__device__ int cuda_split_dfs_match(const int len, const unsigned char *str, const int sequence_type, unsigned int *widths, unsigned char *bids, const barracuda_gap_opt_t *opt, alignment_store_t *aln, int best_score, const int max_aln,uint4 * bwt_occ_array,uint4 * rbwt_occ_array)
					...
					if (score + options_cuda.s_gapo <=worst_tolerated_score) //skip if prospective entry is beyond worst tolerated
					{
						if (e_n_gapo < allow_gap)
						{
							...
							{	//insertions
								...
								unsigned int tmp = (options_cuda.mode & BWA_MODE_LOGGAP)? ((int_log2_cuda(e_n_gape + e_n_gapo))>>1) + 1 : e_n_gapo +
								     e_n_gape;
...
__global__ void cuda_split_inexact_match_caller(int no_of_sequences, unsigned short max_sequence_length, alignment_store_t* global_alignment_store, unsigned char cuda_opt,unsigned int *sequences_array,uint4 * bwt_occ_array,uint4 * rbwt_occ_array,uint2 * sequences_index_array)
...
//and then calls dfs_match to search for alignment using dfs approach
{
	...
	if ( blockId < no_of_sequences )
	{
		...
		for (int i = 0; i < process_length; ++i)
		{
			...
			unsigned char c = read_char(sequence_offset + i + (sequence_info.y- process_length - start_pos), &last_read, &last_read_data,sequences_array );
...
//		}
		...
		if (sequence_type == 1) {
			...
			bwt_cuda_device_calculate_width(local_rc_sequence, sequence_type, local_sequence_widths, local_sequence_bids, process_length,bwt_occ_array,rbwt_occ_array);
			cuda_split_dfs_match(process_length, local_rc_sequence, sequence_type, local_sequence_widths, local_sequence_bids, &local_options, &local_alignment_store, worst_score, max_aln,bwt_occ_array,rbwt_occ_array);
		} else {
#if DEBUG_LEVEL > 6
			printf("normal alignment...");
#endif
			bwt_cuda_device_calculate_width(local_sequence, sequence_type, local_sequence_widths, local_sequence_bids, process_length,bwt_occ_array,rbwt_occ_array);
			cuda_split_dfs_match(process_length, local_sequence, sequence_type, local_sequence_widths, local_sequence_bids, &local_options, &local_alignment_store, worst_score, max_aln,bwt_occ_array,rbwt_occ_array);
		}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
	...
	gettimeofday (&start, NULL);
		...
		hipMalloc((void**)&options, sizeof(barracuda_gap_opt_t));
		hipMemcpy ( options, opt, sizeof(barracuda_gap_opt_t), hipMemcpyHostToDevice);
		hipMemcpyToSymbol(HIP_SYMBOL(options_cuda), opt, sizeof(barracuda_gap_opt_t), 0, hipMemcpyHostToDevice);
		//allocate alignment stores for host and device
		hipMalloc((void**)&global_alignment_store_device, (1ul<<(buffer-3))*sizeof(alignment_store_t));
	...
	while ( ( no_of_sequences = copy_sequences_to_cuda_memory(ks, global_sequences_index, main_sequences_index, global_sequences, main_sequences, &read_size, max_sequence_length, opt->mid, buffer) ) > 0 )
	{
		...
		hipMemcpy (global_alignment_store_device,global_alignment_store_host, no_of_sequences*sizeof(alignment_store_t), hipMemcpyHostToDevice);
		...
		if (split_kernel) {
			hipLaunchKernelGGL((cuda_split_inexact_match_caller), dim3(dimGrid), dim3(dimBlock), 0, 0, no_of_sequences, max_sequence_length, global_alignment_store_device, 0,sequences_array,bwt_occ_array,rbwt_occ_array,sequences_index_array);
		} else {
			fprintf(stderr,"cuda_inexact_match_caller");
			hipLaunchKernelGGL((cuda_inexact_match_caller), dim3(dimGrid), dim3(dimBlock), 0, 0, no_of_sequences, max_sequence_length, global_alignment_store_device, 0,sequences_array,bwt_occ_array,rbwt_occ_array,sequences_index_array);
		}
		...
		do
		{
			...
			fprintf(stderr,".");
#endif
			hipMemcpy (global_alignment_store_host, global_alignment_store_device, no_of_sequences*sizeof(alignment_store_t), hipMemcpyDeviceToHost);
	...
	hipFree(global_alignment_store_device);
    ...
    int device)
{
		...
		if ( bwt != 0 )
		{
			...
			if(mem_available > 0)
			{
				...
				//hipBindTexture(0, bwt_occ_array, *global_bwt, bwt->bwt_size*sizeof(uint32_t));
                                hipMalloc((uint4**)&bwt_occ_array, bwt->bwt_size*sizeof(uint32_t));
				hipMemcpy (bwt_occ_array, *global_bwt , bwt->bwt_size*sizeof(uint32_t), hipMemcpyDeviceToDevice);
		...
		if ( rbwt != 0 )
		{
			...
			if (mem_available > 0)
			{
				...
				//hipBindTexture(0, rbwt_occ_array, *global_rbwt, rbwt->bwt_size*sizeof(uint32_t));
                                hipMalloc((uint4**)&rbwt_occ_array, rbwt->bwt_size*sizeof(uint32_t));
				hipMemcpy (rbwt_occ_array, *global_rbwt , rbwt->bwt_size*sizeof(uint32_t), hipMemcpyDeviceToDevice);
    ...
    hipMalloc((int**)&g_log_n_tex,sizeof(int)*g_log_n_len);
    hipMemcpy (g_log_n_tex, *g_log_n_de , sizeof(int)*g_log_n_len, hipMemcpyDeviceToDevice);
    ...
    hipMalloc((bwtint_t**)&bwt_sa_tex,sizeof(bwtint_t)*bwt->n_sa);
    hipMemcpy (bwt_sa_tex, *bwt_sa_de , sizeof(bwtint_t)*bwt->n_sa, hipMemcpyDeviceToDevice);
    ...
    hipMalloc((bwtint_t**)&rbwt_sa_tex,sizeof(bwtint_t)*rbwt->n_sa);
    hipMemcpy (rbwt_sa_tex, *rbwt_sa_de , sizeof(bwtint_t)*rbwt->n_sa, hipMemcpyDeviceToDevice);
	...
	int device)
{
	...
	hipLaunchKernelGGL((cuda_bwa_cal_pac_pos_parallel2), dim3(dimGrid), dim3(dimBlock), 0, 0, 
		seqs_mapQ_de,
		seqs_pos_de,
...
__device__ uint32_t _bwt_bwt2(bwtint_t k,uint4 * bwt_occ_array)
{
	...
	uint4 four_integers = bwt_occ_array[pos>>2];
...
__device__ uint32_t _rbwt_bwt2(bwtint_t k,uint4 * rbwt_occ_array)
{
    //int pos = (k)/OCC_INTERVAL*12 + 4 + ((k)%OCC_INTERVAL) / 16;
    int pos = (k)/OCC_INTERVAL*12 + 4 + ((k)%OCC_INTERVAL >> 4);
    uint4 four_integers = rbwt_occ_array[pos>>2];
    uint32_t one_integer;
...
__device__ ubyte_t _bwt_B02(bwtint_t k,uint4 * bwt_occ_array)
{
	uint32_t tmp = _bwt_bwt2(k,bwt_occ_array)>>((~(k)&0xf)<<1)&3;
	ubyte_t c = ubyte_t(tmp);
	return c;
}
...
__device__ ubyte_t _rbwt_B02(bwtint_t k,uint4 * rbwt_occ_array)
{
    uint32_t tmp = _rbwt_bwt2(k,rbwt_occ_array)>>((~(k)&0xf)<<1)&3;
    ubyte_t c = ubyte_t(tmp);
    return c;
}
...
__device__
int cuda_bwa_approx_mapQ(const bwa_maxdiff_mapQ_t *p, int mm,int * g_log_n_tex)
{
    int n, g_log;
    if (p->c1 == 0) return 23;
    if (p->c1 > 1) return 0;
    ...
    while (true)
    {
           ...
           (tid < n_sa_in_buf)
        {
            ...
            if (!continuation)
            {
                ...
                uint8_t mapQ = cuda_bwa_approx_mapQ(&maxdiff_mapQ_buf_arr[tid],max_diff,g_log_n_tex);
                ...
                seqs_pos_de[offset+sa_origin[tid]] = sa_return[tid] + bwt_sa_tex[sa_buf_arr[tid]/bwt_sa_intv];
		// Return "mapQ".
        ...
        if (tid < n_sa_in_buf)
        {
            ...
            ubyte_t invPsi2 = _bwt_B02(invPsi1,bwt_occ_array);
            invPsi1 = bwt_cuda_occ(sa_buf_arr[tid],invPsi2,bwt_occ_array);
    ...
    while (true)
    {
           ...
           (tid < n_sa_in_buf)
        {
            ...
            if (!continuation)
            {
                ...
                //seqs_pos_de[offset+sa_origin[tid]] = sa_return[tid] + tex1Dfetch(bwt_sa_tex,sa_buf_arr[tid]/bwt_sa_intv);
                seqs_pos_de[offset+sa_origin[tid]] = bwt_cuda.seq_len - (maxdiff_mapQ_buf_arr[tid].len +
                     sa_return[tid] + rbwt_sa_tex[sa_buf_arr[tid]/rbwt_sa_intv]);
		//sa_return[tid] + tex1Dfetch(rbwt_sa_tex,sa_buf_arr[tid]/rbwt_sa_intv));
		// Return "mapQ".
                seqs_mapQ_de[offset+sa_origin[tid]] = mapQ;
                sa_origin[tid] = -1;
        ...
        if (tid < n_sa_in_buf)
        {
            ...
            ubyte_t invPsi2 = _rbwt_B02(invPsi1,rbwt_occ_array);
            invPsi1 = rbwt_cuda_occ(sa_buf_arr[tid],invPsi2,rbwt_occ_array);
